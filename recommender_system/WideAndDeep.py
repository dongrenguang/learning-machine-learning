...
deep_preprocessed_concat = tf.keras.layers.Concatenate()(deep_preprocessed)
deep_preprocessor = tf.keras.Model(inputs, deep_preprocessed_concat)
deep_body = tf.keras.Sequential([
    tf.keras.layers.Dense(64, activation='relu'),
    tf.keras.layers.Dense(32, activation='relu'),
])
deep_x = deep_preprocessor(inputs)
deep_result = deep_body(deep_x)

wide_preprocessed = [
    tf.reduce_mean(movie_genres_embed(movie_genres_lookup(inputs['movie_genres'])), axis=1, keepdims=False),
]
wide_preprocessed_concat = tf.keras.layers.Concatenate()(wide_preprocessed)
wide_preprocessor = tf.keras.Model(inputs, wide_preprocessed_concat)
wide_x = wide_preprocessor(inputs)
wide_result = tf.keras.layers.Dense(4, activation='relu')(wide_x)

wide_d_deep = tf.keras.layers.Concatenate()([deep_result, wide_result])
wide_d_deep = tf.keras.layers.Dense(1, activation='sigmoid')(wide_d_deep)
model = tf.keras.Model(inputs, wide_d_deep)
