...
user_preprocessed_concat = tf.keras.layers.Concatenate()(user_preprocessed)
user_preprocessor = tf.keras.Model(inputs, user_preprocessed_concat)
user_model = tf.keras.Sequential([
    tf.keras.layers.Dense(64, activation='relu'),
    tf.keras.layers.Dense(32, activation='relu'),
    tf.keras.layers.Dense(16, activation='relu')
])
user_tower = user_preprocessor(inputs)
user_tower = user_model(user_tower)
...
item_preprocessed_concat = tf.keras.layers.Concatenate()(item_preprocessed)
item_preprocessor = tf.keras.Model(inputs, item_preprocessed_concat)
item_model = tf.keras.Sequential([
    tf.keras.layers.Dense(64, activation='relu'),
    tf.keras.layers.Dense(32, activation='relu'),
    tf.keras.layers.Dense(16, activation='relu')
])
item_tower = item_preprocessor(inputs)
item_tower = item_model(item_tower)

result = tf.keras.layers.Dot(axes=1)([user_tower, item_tower])
