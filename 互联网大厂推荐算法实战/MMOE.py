...
class MMOE:
    def __init__(self, expert_dnn_config, num_task, num_expert=None):
        # expert_dnn_config 是一个list，expert_dnn_config[i]是第i个专家的配置
    ...
    def gate(self, unit, deep_fea, name):
        # deep_fea还是底层的输入向量，和喂给专家的是同一个向量
        # unit: 应该是专家的数量
        fea = tf.layers.dense(inputs=deep_fea, units=unit)

        # fea: [B, N], N是专家的个数
        # 代表对于某个task，每个专家的贡献程度
    ...
    def __call__(self, deep_fea):
        ...
        for expert_id in range(self._num_expert):
            # expert_dnn_config是expert_id对应的专家的配置，比如有几层、每一层采用什么样的激活函数
            expert_dnn_config = self._expert_dnn_configs[expert_id]
            expert_dnn = DNN(expert_dnn_config)

            expert_fea = expert_dnn(deep_fea) # 每个专家的输出
            expert_fea_list.append(expert_fea)

        # 假设有N个专家，每个专家的输出是expert_fea，其形状是[B,D]
        # B=batch_size，D=每个专家输出的维度
        ...
        for task_id in range(self._num_task):
            # gate: [B, N]，N是专家的个数，代表对于某个task，每个专家的贡献程度
            gate = self.gate(self._num_expert, deep_fea)
            # gate: 变形成[B, N, 1]
            gate = tf.expand_dim(gate, -1)

            # experts_fea: [B, N, D]
            # gate: [B, N, 1]
            # task_input: [B, N, D]，根据gate给每个专家的输出加权后的结果
            task_input = tf.multiply(experts_fea, gate)
            # task_input: [B, D]，每个专家的输出加权相加
...
# task_input_list[i]是Expert层给第i个task的输入，形状是[B,D]，B=batch_size，D是每个专家的输出维度
...
for i, task_tower_cfg in enumerate(model_config.task_towers):
    ...
    # 比如当前task的名字、task tower有几层、每层的激活函数等
